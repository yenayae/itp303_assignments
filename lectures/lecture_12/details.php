<?php
	...
	if ( !isset($_GET['track_id']) || trim($_GET['track_id']) == '' ) {
		...
		$sql = "SELECT tracks.name AS track, albums.title AS album, composer, milliseconds, bytes, unit_price, media_types.name AS media_type, genres.name AS genre, artists.name AS artist
						FROM tracks
						LEFT JOIN albums
